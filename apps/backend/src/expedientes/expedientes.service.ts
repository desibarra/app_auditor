...
export class ExpedientesService {
    ...
    async uploadFiles(uuid: string, files: Array<Express.Multer.File>) {
        if (!this.s3Service) {
            throw new Error('S3 no está habilitado en este entorno');
        }

        const uploadedFiles: Array<{ name: string; type: string }> = [];
    ...
    async getProgress(uuid: string) {
        const query = `
            SELECT tipo_gasto,
                ...
                (CASE WHEN EXISTS (
                    SELECT 1
                    FROM archivo_expediente
                    WHERE uuid_expediente = $1
                      AND tipo = 'contrato'
                ) THEN 40 ELSE 0 END) +
                (CASE WHEN EXISTS (
                    SELECT 1
    ...
    async recalculateRisk(uuid: string) {
        const query = `
            ...
            WHERE uuid_expediente = $1
              AND tipo_alerta = 'CFDI sin materialidad';
        `;
